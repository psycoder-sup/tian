---
name: tian architecture context
description: Key architecture patterns for tian - SwiftUI terminal emulator with @MainActor @Observable models, NotificationCenter events, Process+DispatchQueue subprocess pattern
type: project
---

tian is a macOS terminal emulator using SwiftUI + Ghostty. Key architecture patterns discovered during spec review:
...
- **Logger**: `Log` enum in `tian/Utilities/Logger.swift` with categories: core, view, ghostty, persistence, lifecycle, perf, ipc, worktree
