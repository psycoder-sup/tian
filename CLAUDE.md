...
Cascading close via `onEmpty` / `onSessionClose` callbacks: `PaneViewModel` → `Session` → `SessionCollection`. A Claude pane exit (or ⌘W on the Claude pane) closes the session; emptying the terminal panel just drops the panel. Closing a workspace's **last** session does *not* close the workspace — it stays alive and its content area shows a create-session empty state (`SessionEmptyStateView`); a workspace closes only on an explicit gesture (sidebar Close / ⌘-close-workspace → `WorkspaceCollection.removeWorkspace`).
...
- **Now:** v1.5.8 released (notarized, appcast live) — overview selected-card glow-clip fix, skill-install scoping. Daily-drive and triage regressions. See `status.json`.
- **Next:** Remote-workspace phase 2 (IPC socket forwarding + remote shell integration); validate the /tian implement harness on a real multi-slice run (ADR 0003).
