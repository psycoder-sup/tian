...
struct IPCResponse: Codable, Sendable {
    ...
    static func success(_ result: [String: IPCValue] = [:]) -> IPCResponse {
        IPCResponse(version: ipcProtocolVersion, ok: true, result: result, error: nil)
    }

    static func failure(code: Int, message: String) -> IPCResponse {
        IPCResponse(version: ipcProtocolVersion, ok: false, result: nil, error: IPCError(code: code, message: message))
    }
}
