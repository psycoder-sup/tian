...
**Based on:** docs/feature/aterm/aterm-prd.md v1.4 (Milestone 1)
**Author:** CTO Agent
**Date:** 2026-03-24
**Version:** 1.0
**Status:** Draft
...
|-------|---------------|--------------|-----------|
| **App Shell** | SwiftUI window, menu bar, toolbar | Main thread | `AtermApp`, `TerminalWindow`, `TerminalView` |
...
aterm/
  ...
  aterm/
    App/
      AtermApp.swift                 -- @main, app lifecycle, menu bar
...
`GhosttyBridge` provides a Swift-idiomatic interface over the raw C API. It is **not** a general-purpose wrapper -- it exposes only the subset of libghostty-vt used by aterm. Key design principles:
...
The window's title displays the shell's current working directory or a static title like "aterm" for M1.
...
| 2 | Does libghostty-vt provide selection/selection-text-extraction APIs, or must aterm implement selection entirely? | The render state API exposes cell data, but selection tracking (anchor/extent, word detection) may or may not be in the library. | If not provided, aterm must implement word boundary detection and selection coordinate tracking (specced above as a mitigation). |
