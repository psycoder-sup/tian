# Spec Validation Report: aterm Milestones M1-M7
...
- **Severity:** Minor
- **Detail:** Each spec proposes its own directory layout. M1 uses `aterm/aterm/{layer}/` (e.g., `Core/`, `Renderer/`, `Selection/`). M2 uses `aterm/Sources/aterm/Features/Pane/`. M3 uses `aterm/Sources/App/`, `Models/`, `Views/`, `Input/`. M4 uses `Models/`, `Views/`, `Utilities/`. M6 uses `Sources/aterm/Configuration/`, `Keybinding/`, `Theme/`. These are inconsistent -- some use feature-based, some use layer-based organization.
