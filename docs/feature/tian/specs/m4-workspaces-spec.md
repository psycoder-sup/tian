...
- **`TianApp.swift`** — Uses a minimal SwiftUI `App` scene with `MenuBarExtra` or `.commands {}` for menu bar items and keyboard shortcuts. The initial window is created programmatically via `WindowCoordinator` on launch, not via `WindowGroup`.
...
All paths below are relative to the project's Swift source root (e.g., `tian/Sources/`). The exact root depends on whether the project uses Swift Package Manager or an Xcode project; the relative structure within is what matters.
...
WindowManagement/
    ...
    TianAppDelegate.swift           -- NSApplicationDelegate for quit handling, window restoration
...
App/
    TianApp.swift               -- (modified) Minimal SwiftUI App scene for menu bar commands; windows created via WindowCoordinator
```
...
Shortcuts should be registered via the main menu bar (built in `TianAppDelegate` or via SwiftUI `.commands { }` if using a hybrid App scene), making them discoverable via Help > Search. The `WorkspaceWindowController` can also register shortcuts as `NSEvent` local monitors for window-scoped actions.
...
- Implement `TianAppDelegate` with `applicationShouldTerminate(_:)` for quit handling.
...
| Window close (red button) vs. app quit semantics -- macOS convention is that closing the last window does not quit the app, but tian follows FR-05 cascading close | Low -- developer tools commonly quit on last window close (e.g., Terminal.app) | Low | Closing the last workspace quits the app via `NSApplication.shared.terminate(nil)`. The M5 quit flow (serialize state) still runs. This matches Terminal.app behavior and FR-05 requirements. |
