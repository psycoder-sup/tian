...
All persistence files live in `~/Library/Application Support/tian/`. This directory is created on first quit-save if it does not exist. The primary state file is `state.json`. A backup of the previous state is kept as `state.prev.json` (overwritten each save cycle).
...
1. **Check for state file.** Look for `~/Library/Application Support/tian/state.json`.
...
This data is logged locally only (no external telemetry per PRD non-goal NG2). It can be viewed via Console.app by filtering on the tian subsystem.
...
The app writes to `~/Library/Application Support/tian/`, which is accessible by the app's sandbox (if sandboxed) or freely for non-sandboxed apps. Since this is a developer tool not targeting the App Store (NG6), sandboxing is not required, and no entitlements are needed for this directory.
...
Introduce a `persistenceEnabled` flag (default: true, overridable via a launch argument `--no-persistence` or an environment variable `TIAN_NO_PERSISTENCE=1`). When disabled:
...
| File permissions on `~/Library/Application Support/tian/` prevent writes | Serialization fails silently; state not saved | Very low | Check and create directory with correct permissions at app start. Log errors clearly. |
