...
**Based on:** docs/feature/tian/tian-prd.md v1.4 (FR-26 through FR-31, FR-43 pane resize config)
**Author:** CTO Agent
**Date:** 2026-03-24
**Version:** 1.0
**Status:** Draft
...
|----------|------|---------|
| 1 (highest) | `~/.config/tian/config.toml` | Primary user configuration |
| 2 (fallback) | `~/Library/Application Support/tian/config.toml` | macOS-conventional location |
...
| Path | Purpose |
|------|---------|
| `~/.config/tian/themes/` | Directory for user-defined theme TOML files (one file per theme) |
...
| `theme` | String | `"tian-dark"` | Name of the color theme to use |
...
The keybinding system translates macOS keyboard events into tian actions. It replaces any hardcoded keyboard handling from M1-M5 with a configurable, data-driven dispatch.
...
- tian keybindings take priority over terminal input. If `cmd+c` is bound to copy, the terminal never sees `cmd+c`.
...
- System-level shortcuts (Cmd+Q, Cmd+H, Cmd+M) are handled by macOS before tian sees them. The keybinding system does not override these.
...
2. **User theme files** in `~/.config/tian/themes/<name>.toml`
...
|------------|-------|-------------|
| `tian-dark` | Dark | Default dark theme, similar to Tokyo Night / One Dark aesthetic |
| `tian-light` | Light | Default light theme for bright environments |
...
|------|---------------|---------|
| Config file (e.g., `~/.config/tian/config.toml`) | `.write`, `.rename`, `.delete` | Detect config edits |
| User themes directory (`~/.config/tian/themes/`) | `.write` | Detect theme file changes |
...
Resources/
  themes/
    tian-dark.toml                  -- Bundled dark theme
    tian-light.toml                 -- Bundled light theme
...
- Users can copy this file to `~/.config/tian/config.toml` to begin customizing
...
- Create bundled `tian-dark.toml` and `tian-light.toml` theme files
