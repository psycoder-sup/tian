...
Worktree Spaces is an aterm feature that automates the creation of git worktree-backed development environments. When triggered, aterm creates a new git worktree on disk, creates a new Space pointed at that worktree directory, copies specified environment files from the main worktree, runs setup scripts, and applies a predefined pane layout with startup commands. The feature is driven by a per-project configuration file (`.aterm/config.toml`) checked into the repository root. When a worktree Space is closed, aterm offers to remove the associated worktree from disk, completing the lifecycle. The feature is available both as a built-in app command (keyboard shortcut, sidebar context menu) and as a CLI command (`aterm-cli worktree create`) over the existing IPC system.

---

## 2. Problem Statement

**User Pain Point:** When a developer needs to work on a new branch in parallel -- a hotfix, a code review, or an experimental feature -- they must manually perform a sequence of steps: run `git worktree add`, copy `.env` files and other untracked configuration, run setup scripts (`npm install`, `bundle install`, etc.), create a new Space in aterm, and manually split panes and run startup commands (server, watcher, test runner). This sequence is error-prone (forgetting to copy `.env` breaks the app, forgetting a setup step wastes debugging time) and slow enough to discourage parallel work.

**Current Workaround:** Developers either (a) run all these steps manually each time, (b) maintain personal shell scripts that partially automate the sequence but have no integration with aterm's Space model, or (c) avoid parallel worktrees altogether and use `git stash` / branch switching instead, losing the ability to keep multiple branches open simultaneously.

**Business Opportunity:** aterm's 4-level hierarchy explicitly maps Space to "branch/worktree" (per the main PRD and the project memory). Today, creating a worktree-backed Space requires manual coordination between git, the filesystem, and aterm's UI. Automating this sequence makes the Space-as-worktree concept a first-class, one-command workflow -- the primary differentiator that justifies aterm's hierarchy over a flat tab model. It transforms the conceptual model into a practical daily-driver feature.
...
| 4 | developer | close a worktree Space and have aterm clean up the worktree directory on disk | I don't accumulate stale worktree directories that I have to manually prune |
| 5 | developer | run `aterm-cli worktree create` from a script or AI agent within aterm | worktree creation can be automated as part of larger workflows (e.g., Claude Code setting up a review environment) |
...
**FR-001:** aterm must look for a per-project worktree configuration file at `.aterm/config.toml` relative to the git repository root of the current Space's working directory. This file defines env file copy rules, setup scripts, and the default pane layout for new worktree Spaces in that repository.

**FR-002:** If no `.aterm/config.toml` file exists in the repository root, the worktree creation flow must still function -- it creates the worktree and Space with a single pane and no file copying or setup scripts. The config file enhances the flow but is not required.
...
**FR-006:** When triggered, aterm must prompt the user for a branch name (or accept it as a CLI argument). The branch name is used as both the git worktree branch name and the Space name.

**FR-007:** aterm must determine the git repository root from the current Space's working directory (or from a CLI-provided path). If the current directory is not inside a git repository, the operation must fail with a clear error message.

**FR-008:** aterm must create the git worktree by running `git worktree add <worktree-path> -b <branch-name>` (new branch) or `git worktree add <worktree-path> <existing-branch>` (existing branch). The user must be able to specify whether to create a new branch or check out an existing one.

**FR-009:** The worktree directory must be created at `<repo-root>/.worktrees/<branch-name>` by default. This path must be configurable in `.aterm/config.toml` via a `worktree_dir` setting (relative to repo root).

**FR-010:** After the worktree is created on disk, aterm must copy all files matching the env file rules defined in FR-003 from the git repository's **main worktree** to the new worktree, preserving relative paths. The main worktree path must be resolved by parsing the output of `git worktree list --porcelain` (the first listed worktree is the main worktree), NOT derived from the current Space's working directory. This ensures consistent env files regardless of which Space triggers the creation.

**FR-011:** After file copying, aterm must create a new Space in the current Workspace with a **single pane**. The Space's `defaultWorkingDirectory` must be set to the new worktree path. The Space name must be set to the branch name. aterm must activate (focus) the new Space immediately so the user can see setup output in real time.

**FR-012:** After the single-pane Space is visible, aterm must execute each setup command defined in FR-004 sequentially **inside the visible pane** (typed into the terminal per FR-028), so the user can observe output in real time. aterm waits for shell readiness (FR-028) between each command before typing the next. aterm does not attempt to detect exit codes from interactively-typed commands. If the user observes a failure, they can cancel remaining setup via the cancel button or Ctrl+C (FR-014). The Space remains open so the user can debug.

**FR-013:** After all setup commands complete successfully (or if no config file exists), aterm must apply the layout defined in the config file to the first tab. This means constructing the `SplitTree` with the specified split structure and ratios, and sending each pane's startup command to its terminal after shell readiness is detected (per FR-028). If setup was cancelled (FR-014), layout application still proceeds.
...
**FR-015:** When a worktree-backed Space is closed (via sidebar, keyboard shortcut, or CLI), aterm must offer to remove the associated git worktree from disk. The user must be presented with a confirmation that includes the worktree path and branch name.

**FR-016:** If the user confirms cleanup, aterm must run `git worktree remove <worktree-path>` to cleanly remove the worktree. If the worktree has uncommitted changes, `git worktree remove` will fail; aterm must surface this error and offer a force option (`git worktree remove --force`).

**FR-017:** aterm must track the association between a Space and its worktree path so that cleanup can be offered on Space close. This association must persist across app launches (via the existing session persistence system).

### CLI Surface

**FR-018:** The CLI must support a `worktree create` subcommand: `aterm-cli worktree create <branch-name> [--existing] [--path <repo-path>]`. The `--existing` flag checks out an existing branch instead of creating a new one. The `--path` flag overrides the repository root detection (defaults to the current Space's working directory via `ATERM_SPACE_ID`).

**FR-019:** The CLI must support a `worktree remove` subcommand: `aterm-cli worktree remove <space-id> [--force]`. This closes the Space and removes the worktree from disk. The `--force` flag bypasses the uncommitted-changes check.

**FR-020:** The CLI `worktree create` command must return the Space UUID on success, enabling scripts to chain subsequent commands (e.g., `aterm-cli space focus <id>`). The response must include an `"existed": boolean` field indicating whether the Space was newly created or an existing duplicate was focused (per FR-027).
...
**FR-024:** When triggered via the app (shortcut or context menu), aterm must present a minimal input UI for the branch name and a toggle for "existing branch" vs. "new branch". This UI should not block the terminal -- it can be a popover, sheet, or inline sidebar input.
...
**FR-027:** When a worktree Space creation is requested, aterm must check all existing Spaces for a matching `worktreePath`. If a Space with the same worktree path already exists, aterm must **focus that Space** instead of creating a new one. The IPC response must return the existing Space's UUID with an `"existed": true` flag so callers can distinguish between newly created and already-existing Spaces.

### Shell Readiness Detection

**FR-028:** Before typing startup commands (layout `command` fields) or setup commands into a pane's terminal, aterm must detect that the shell is ready. The primary signal is **OSC 7** (Operating System Command for current working directory notification), which modern shells emit after each prompt. If OSC 7 is not received within a configurable fallback delay, aterm must proceed after that delay expires. The fallback delay must be configurable in `.aterm/config.toml` via a `shell_ready_delay` setting (seconds, default: `0.5`).

### Worktree Cleanup: Directory Pruning

**FR-030:** After `git worktree remove` succeeds, aterm must prune empty parent directories upward from the removed worktree path up to (but not including) the `worktree_dir` root. This handles branch names containing slashes (e.g., `feature/payment-api` creates `.worktrees/feature/payment-api/`): after removal, the empty `feature/` directory must be cleaned up. Non-empty parent directories must not be removed.
...
**FR-033:** During worktree creation, aterm must check the repository's `.gitignore` for the configured `worktree_dir` value (default: `.worktrees`). If the value is not already present, aterm must append it to `.gitignore` with a preceding comment line `# aterm worktree directory`. If `.gitignore` does not exist, aterm must create it. This prevents worktree directories from being accidentally committed.
...
**NFR-003:** The `.aterm/config.toml` config file must be validated on read. Parsing errors must produce a clear, actionable error message including the line number and expected format. Invalid config must not prevent the basic worktree creation flow (FR-002 applies).
...
**NFR-005:** The feature must work correctly when the repository already has existing worktrees (created outside aterm). It must not interfere with worktrees it did not create.
...
Precondition: User is in a Space whose working directory is inside a git repository.
              The repository contains `.aterm/config.toml`.

1. User presses the worktree creation shortcut (e.g., Cmd+Shift+B).
2. aterm presents a branch name input (popover or inline).
...
4. aterm checks for an existing Space with the same worktree path.
   - If found: focuses the existing Space and stops (FR-027).
5. aterm shows a progress indicator in the sidebar area.
6. aterm resolves the git repo root from the current Space's working directory.
7. aterm reads and parses `.aterm/config.toml` from the repo root.
8. aterm runs: git worktree add .worktrees/feature/payment-api -b feature/payment-api
9. aterm copies env files from the main worktree per config (e.g., .env, .env.local).
   Source is always the main worktree (resolved via `git worktree list --porcelain`).
10. aterm creates a new Space named "feature/payment-api" in the current Workspace
...
11. aterm waits for shell readiness (OSC 7 or fallback delay per FR-028).
12. aterm runs setup commands sequentially inside the visible pane (e.g., npm install).
...
13. After setup completes (or is cancelled), aterm applies the layout from config:
...
2. aterm detects the Space has an associated worktree.
3. aterm presents a confirmation dialog:
...
5. aterm runs: git worktree remove .worktrees/feature/payment-api
6. aterm prunes empty parent directories up to the worktree_dir root (FR-030).
   (e.g., removes empty .worktrees/feature/ after removing .worktrees/feature/payment-api)
7. aterm removes the Space from the SpaceCollection.
8. Sidebar updates.
```
...
- **CLI trigger:** Steps 1-3 are replaced by `aterm-cli worktree create feature/payment-api`. Step 4 still applies (returns existing Space UUID with `"existed": true` if duplicate). No UI prompt for branch name.
...
- **Setup command failure:** aterm does not detect setup command exit codes (commands are typed interactively). If the user observes a failure, they can cancel remaining setup via the floating cancel button or Ctrl+C (FR-014). The Space is already visible (FR-012) and layout application still proceeds (FR-013).
- **Uncommitted changes on remove:** "Cannot remove worktree: uncommitted changes. Force remove?" [Force Remove] [Cancel]
- **Config parse error:** "Error in .aterm/config.toml line 12: expected string for 'source'. Proceeding without config." (Falls back to FR-002 behavior.)
- **Unhandled git errors:** Any git error from `git worktree add` (or `git worktree remove`) that is not covered by the cases above is surfaced verbatim to the user, alongside aterm context: the branch name, worktree path, and the full command that was run. This ensures no git failure is silently swallowed.
...
The config file lives at `.aterm/config.toml` in the git repository root. This keeps it version-controlled and project-specific while using the `.aterm` directory as a namespace for future aterm project-level config.
...
The layout uses TOML's nested table syntax to define a binary split tree that maps directly to aterm's existing `PaneNode` / `SplitTree` model:
...
- **Automatic branch detection:** aterm does not watch for branch changes or auto-create Spaces. All worktree creation is user-initiated.
...
- **Config file creation wizard:** The user writes `.aterm/config.toml` manually. No interactive setup.
...
Since aterm is a personal tool (single developer, no telemetry), success is measured qualitatively:
...
- ~~OQ#2~~ aterm auto-appends `worktree_dir` to `.gitignore` if not already present. See FR-033.
