...
final class IPCServer: Sendable {
    static var socketPath: String {
        ...
        return "\(tmpdir)tian\(suffix)-\(getuid()).sock"
    }

    private let commandHandler: @Sendable (IPCRequest) async -> IPCResponse
    private let queue = DispatchQueue(label: "com.tian.ipc-server", qos: .userInitiated)
    ...
    func start() {
        ...
        if FileManager.default.fileExists(atPath: path) {
            if isSocketAlive(path: path) {
                Log.ipc.warning("Another tian instance is listening on \(path); taking over")
            }
            unlink(path)
        }
