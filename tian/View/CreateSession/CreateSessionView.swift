...
/// Unified modal for creating a session — with or without an associated git worktree.
...
struct CreateSessionView: View {
    ...
    /// Plain-session submit. The name is optional — a `nil` name creates an
    /// auto-named session (the field placeholder is "Session name (optional)").
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 12) {
                ...
                // worktree, so there's no branch/session name to type.
        ...
        HStack(spacing: 6) {
            if claudeWorktreeMode {
                ...
                if !sanitizedInput.isEmpty {
                    Text("Will create plain session \u{201C}\(sanitizedInput)\u{201D}")
                }
            } else if !isGitRepo {
    ...
    static func resolveSubmitAction(
        ...
        // Plain (non-worktree) mode: the session name is optional. An empty
        // field creates an auto-named session, so it submits rather than
        // blocking. Checked before the empty guard, which only gates worktree mode.
        if !worktreeEnabled {
            return .plain(name: sanitizedInput.isEmpty ? nil : sanitizedInput)
        }
        // Worktree mode requires a branch name.
...
enum SubmitAction: Equatable {
    ...
    case claudeWorktree
    /// Plain session. `name == nil` means an auto-named session (the name
